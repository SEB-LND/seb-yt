...
import { isSidebarDrawerOpenAtom, playlistAtom } from "../store";
...
const PlaylistPage = () => {
  ...
  // Playlist videos
  const [playlistVideos] = useAtom(playlistAtom);
  const [filteredVideos, setFilteredVideos] = useState([]);

  useEffect(() => {
    setFilteredVideos(playlistVideos ? [...playlistVideos] : []);
  }, [playlistVideos]);
  ...
  if (!filteredVideos || filteredVideos.length === 0) {
    return (
      <EmptyMessage>
        ...
        <p>Videos you add to your playlist will appear here.</p>
      </EmptyMessage>
    );
  }
